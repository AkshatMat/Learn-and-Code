    ...
    def _process_single_article(self, article: NewsArticle) -> bool:
        try:
            ...
            if db_success:
                ...
                notification_success = self.db_repository.process_article_notifications(article)
                if notification_success:
                    logger.info(f"Notifications processed for article: {article.id}")
                else:
                    logger.warning(f"Failed to process notifications for article: {article.id}")
            else:
